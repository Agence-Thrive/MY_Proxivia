<template>
    <div class="content">
        <b-row class="row-content">
            ...
            <b-col>
                <button
                    type="submit"
                    v-b-modal.modal-1
                    class="button-box btn-outline-primary float-right"
                >
                    Ajouter un prospect
                </button>
            </b-col>
        ...
        <b-modal ok-only hide-footer size="xl" title="" id="modal-1">
            <sectionAddProspect></sectionAddProspect>
        </b-modal>
        ...
        <b-row class="row-content">
            <b-col>
                ...
                >
                    ...
                    <template
                        v-if="actualProspects[0]"
                        v-slot:cell(houseValue)="row"
                    >
                        {{ row.value }}€
                    </template>
                    <template
                        v-if="actualProspects[0]"
                        v-slot:cell(commission)="row"
                    >
                        {{ row.value }}€
                    </template>
...
export default {
    ...
    mounted() {
        ...
        for (const prospect in this.info.prospects) {
            axios
                .get(
                    `${process.env.VUE_APP_ROOT_API}/api/v1/users/${this.info._id}/prospect/${this.info.prospects[prospect]}`,
                    {
                        headers: {
                ...
                .then((response) => {
                    this.actualProspects = this.actualProspects.concat(
                        response.data
                    );
                });
        }
